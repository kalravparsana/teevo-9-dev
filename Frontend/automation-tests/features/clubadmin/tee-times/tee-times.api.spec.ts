...
test.describe('Tee Time Slots — API Mock Tests', () => {
  ...
  test('shows graceful UI on server error (500)', async ({ page }) => {
    await page.route('**/api/tee-times', (route) =>
      route.fulfill({
        status: 500,
        body: JSON.stringify({ message: teeTimeData.api.errorMessage ?? 'Internal Server Error' }),
      }),
    )
    const app = new TeevoAppPage(page)
    await app.goto()
  ...
  test('empty API response does not crash page', async ({ page }) => {
    await page.route('**/api/tee-times', (route) =>
      route.fulfill({
        status: 200,
        contentType: 'application/json',
        body: JSON.stringify(teeTimeData.api.listEmpty ?? []),
      }),
    )
    const app = new TeevoAppPage(page)
    await app.goto()
