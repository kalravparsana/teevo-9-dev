...
for (const viewport of MOBILE_VIEWPORTS) {
  test.describe(`Tee Time Slots — Mobile: ${viewport.name}`, () => {
    ...
    test('page renders correctly on mobile viewport', async ({ page }) => {
      ...
      await expect(page.getByRole('heading', { level: 1 })).toHaveText('Tee Time Slots')
    })
    ...
    test('primary content is in viewport', async ({ page }) => {
      await expect(page.getByText('Tee Time Configuration')).toBeInViewport()
    })
