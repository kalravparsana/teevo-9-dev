...
test.describe('Operating Hours — API Mock Tests', () => {
  ...
  test('shows graceful UI on server error (500)', async ({ page }) => {
    await page.route('**/api/operating-hours', (route) =>
      route.fulfill({
        status: 500,
        body: JSON.stringify({ message: operatingHoursData.api.errorMessage ?? 'Internal Server Error' }),
      }),
    )
    const app = new TeevoAppPage(page)
    await app.goto()
  ...
  test('handles network failure without crash', async ({ page }) => {
    await page.route('**/api/operating-hours', (route) => route.abort('failed'))
    const app = new TeevoAppPage(page)
    await app.goto()
    await app.navigate('Club Admin', 'Operating Hours')
    await expect(app.mainPanel).toBeVisible()
  ...
  test('empty API response does not crash page', async ({ page }) => {
    await page.route('**/api/operating-hours', (route) =>
      route.fulfill({
        status: 200,
        contentType: 'application/json',
        body: JSON.stringify(operatingHoursData.api.listEmpty ?? []),
      }),
    )
    const app = new TeevoAppPage(page)
    await app.goto()
