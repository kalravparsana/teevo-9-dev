...
for (const viewport of MOBILE_VIEWPORTS) {
  test.describe(`Operating Hours — Mobile: ${viewport.name}`, () => {
    ...
    test('page renders correctly on mobile viewport', async ({ page }) => {
      ...
      await expect(page.getByRole('heading', { level: 1 })).toHaveText('Operating Hours')
    })
    ...
    test('primary content is in viewport', async ({ page }) => {
      await expect(page.getByText('Club Operating Hours')).toBeInViewport()
    })
