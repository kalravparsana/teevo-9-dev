...
for (const viewport of MOBILE_VIEWPORTS) {
  test.describe(`Add Club — Mobile: ${viewport.name}`, () => {
    ...
    test('page renders correctly on mobile viewport', async ({ page }) => {
      ...
      await expect(page.getByRole('heading', { level: 1 })).toHaveText('Add Club')
    })
    ...
    test('primary content is in viewport', async ({ page }) => {
      await expect(page.getByText('Add New Club')).toBeInViewport()
    })
