...
for (const viewport of MOBILE_VIEWPORTS) {
  test.describe(`Manage Users — Mobile: ${viewport.name}`, () => {
    ...
    test('page renders correctly on mobile viewport', async ({ page }) => {
      ...
      await expect(page.getByRole('heading', { level: 1 })).toHaveText('Manage Users')
    })
    ...
    test('primary content is in viewport', async ({ page }) => {
      await expect(page.getByText('Add User')).toBeInViewport()
    })
