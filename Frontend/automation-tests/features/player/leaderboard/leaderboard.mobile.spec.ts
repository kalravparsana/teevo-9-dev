...
for (const viewport of MOBILE_VIEWPORTS) {
  test.describe(`Tournament Leaderboard — Mobile: ${viewport.name}`, () => {
    ...
    test('page renders correctly on mobile viewport', async ({ page }) => {
      ...
      await expect(page.getByRole('heading', { level: 1 })).toHaveText('Tournament Leaderboard')
    })
    ...
    test('primary content is in viewport', async ({ page }) => {
      await expect(page.getByText('Tournament Leaderboard')).toBeInViewport()
    })
