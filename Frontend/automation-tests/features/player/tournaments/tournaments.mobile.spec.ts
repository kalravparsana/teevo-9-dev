...
for (const viewport of MOBILE_VIEWPORTS) {
  test.describe(`Upcoming Tournaments — Mobile: ${viewport.name}`, () => {
    ...
    test('page renders correctly on mobile viewport', async ({ page }) => {
      ...
      await expect(page.getByRole('heading', { level: 1 })).toHaveText('Upcoming Tournaments')
    })
    ...
    test('primary content is in viewport', async ({ page }) => {
      await expect(page.getByText('Upcoming Tournaments')).toBeInViewport()
    })
